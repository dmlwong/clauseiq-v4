...
export function ComparisonSummaryRail({
  ...
  const metricCounts = metrics ?? {
    ...
    met: comparison.met,
    partiallyMet: comparison.improved,
...
const metricDefinitions: Array<{
  ...
  { key: "partially-met", label: "Partially Met", value: "partiallyMet", tone: "warning", group: "changes" },
  { key: "not-met", label: "Not met", value: "notMet", tone: "destructive", group: "changes" },
  { key: "worsened", label: "Worsened", value: "worsened", tone: "destructive", group: "changes" },
  { key: "unexpected", label: "Unexpected", value: "unexpected", tone: "warning", group: "changes" },
  { key: "manual-review", label: "Manual Review", value: "manualReview", tone: "destructive", group: "changes" },
...
function MetricGrid({
  ...
  const renderMetricRow = (definition: (typeof metricDefinitions)[number], label = definition.label) => {
    ...
    return (
      <ToggleCard
        key={definition.key}
        status={disabled ? "Disabled" : active ? "Selected" : "Default"}
        aria-pressed={active}
  ...
  if (grouped) {
    return (
      <div className="mt-orbit-base grid gap-orbit-base lg:grid-cols-2">
        {(["changes", "risk"] as const).map((group) => (
          <div key={group} className="rounded-lg border border-border/70 bg-white/60 p-orbit-s">
            <Text as="p" size="Small" variant="Secondary">{group === "changes" ? "Changes" : "Deviation Level"}</Text>
            <div className="grid grid-cols-2 gap-orbit-s">
  ...
  if (density === "rail") {
    const changeMetrics = metricDefinitions.filter((definition) => definition.group === "changes");
    const riskMetrics = metricDefinitions.filter((definition) => definition.group === "risk");
      ...
      <div className="mt-orbit-base">
        <div className="space-y-orbit-base">
          <div>
            ...
            <div className="space-y-orbit-xs">
              {changeMetrics.map((definition) => renderMetricRow(definition))}
            </div>
          </div>
          <div>
            <div className="mb-orbit-xs rounded-md py-orbit-xs">
            ...
            <div className="space-y-orbit-xs">
              {riskMetrics.map((definition) => renderMetricRow(definition))}
            </div>
          </div>
        </div>
      </div>
