...
import { InitiativeModal } from "@/components/clauseiq-v7/InitiativeModal";
import {
  CIQ_INITIATIVES,
  type CiqInitiative,
} from "@/lib/clauseiq-v7-data";
import { cn } from "@/lib/utils";
import { V4_DELIVERY_INITIATIVE_ID } from "@/data/mock-delivery-engine-v7";
import { SupplierOutputsPanel } from "@/components/clauseiq-v7/supplier-results";
import type { ResultsLayout, SupplierOutputSelection } from "@/components/clauseiq-v7/supplier-results/types";
import {
  ClauseIqJourneyContent,
  type ClauseIqJourneyRefs,
} from "@/components/clauseiq-v7/ClauseIqJourney";
import {
  LATEST_V6_RESULTS_ROUTE,
  createDefaultParameterSelection,
  useClauseIqWorkflow,
  type ClauseIqWorkflowStep,
} from "@/components/clauseiq-v7/ClauseIqWorkflow";
import {
  getSupplierOutputComparisonContext,
  mockInitiative,
} from "@/data/mock-clauseiq-v7";

interface ClauseIQV7Props {
  forceResults?: boolean;
  resultsLayout?: ResultsLayout;
}

export default function ClauseIQV7({ forceResults = false, resultsLayout = "accordion" }: ClauseIQV7Props) {
  ...
  const handleRunAgain = useCallback(() => {
    if (!forceResults) {
      ...
      navigate(`/clauseiq-v7/output-panel?${nextParams.toString()}`);
    }
  ...
  useEffect(() => {
    if (!forceResults && searchParams.get("view") === "results" && !rerunUploadFromRoute) {
      navigate("/clauseiq-v7/output-panel", { replace: true });
      return;
    }
  ...
  const handleViewResult = (selection?: SupplierOutputSelection) => {
    ...
    if (!context) {
      ...
      navigate(`/initiatives-v7?${fallbackParams.toString()}`);
      return;
    }
    ...
    navigate(`/initiatives-v7?${params.toString()}`);
  };
