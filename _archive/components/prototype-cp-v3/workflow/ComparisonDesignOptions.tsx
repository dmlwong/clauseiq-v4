...
import { ArrowRight, ClipboardList, Columns3, Info, List, Sigma } from "@/components/prototype-cp-v3/clauseiq/v6aIcons";
import {
  Badge,
  Card,
  Chip,
    ...
    return (
      <div className="mx-auto w-full max-w-[1500px] space-y-orbit-base px-orbit-base py-orbit-base">
        ...
        <div className="grid gap-orbit-base xl:grid-cols-[304px_minmax(0,1fr)] xl:items-start">
          <aside className="prototype-cp-v3-comparison-sticky-rail xl:self-start">
            <section className="prototype-cp-v3-comparison-sticky-rail-panel flex overflow-hidden rounded-orbit-lg border border-orbit-border bg-orbit-card xl:flex-col">
...
function ScoreHero({
  ...
  return (
    <div className={cn("prototype-cp-v3-score-hero", className)}>
      <Card type="Static" padding={compact ? "Small" : "Base"} state="Accent">
        <div className="space-y-orbit-s px-orbit-xs py-orbit-xs">
          <div className="flex min-w-0 items-center justify-between gap-orbit-base">
...
const v6aDisabledToggleCardClassName = "prototype-cp-v3-togglecard-disabled";
  ...
  const renderMetricRow = (definition: (typeof firstAnalysisMetricDefinitions)[number]) => {
    ...
    return (
      <ToggleCard
        ...
        className={cn("overflow-hidden", disabled ? v6aDisabledToggleCardClassName : !active && "prototype-cp-v3-togglecard-subtle")}
