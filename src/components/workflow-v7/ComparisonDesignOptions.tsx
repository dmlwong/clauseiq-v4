...
import { ArrowRight, Columns3, Info, List } from "@/components/clauseiq-v7/v7Icons";
import {
  Badge,
  Card,
  Chip,
    ...
    return (
      <div className="mx-auto w-full max-w-[1500px] space-y-orbit-base px-orbit-base py-orbit-base">
        ...
        <div className="grid gap-orbit-base xl:grid-cols-[320px_minmax(0,1fr)] xl:items-start">
          <aside className="clauseiq-v7-comparison-sticky-rail xl:self-start">
            <section className="clauseiq-v7-comparison-sticky-rail-panel flex overflow-hidden rounded-orbit-lg border border-orbit-border bg-orbit-card xl:flex-col">
...
function ScoreHero({
  ...
  return (
    <div className={cn("clauseiq-v7-score-hero", className)}>
      <Card type="Static" padding={compact ? "Small" : "Base"} state="Accent">
        <div className="space-y-orbit-s px-orbit-xs py-orbit-xs">
          <div className="flex min-w-0 items-center justify-between gap-orbit-base">
...
type V7ToggleCardStatus = "Default" | "Hover" | "Selected" | "Disabled" | "Subtle";
const v7DisabledToggleCardClassName = "clauseiq-v7-togglecard-disabled";

function getV7ToggleCardStatus({ active, disabled }: { active: boolean; disabled: boolean }): V7ToggleCardStatus {
...
function FirstAnalysisMetricGrid({
  ...
  const renderMetricRow = (definition: (typeof firstAnalysisMetricDefinitions)[number]) => {
    ...
    const toggleCardStatus = getV7ToggleCardStatus({ active, disabled });
    ...
    return (
      <ToggleCard
        ...
        className={cn("overflow-hidden", disabled ? v7DisabledToggleCardClassName : !active && "clauseiq-v7-togglecard-subtle")}
