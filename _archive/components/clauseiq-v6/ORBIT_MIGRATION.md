# ClauseIQ V6 Orbit Migration Notes
...
- `button`, `badge`, `input`, `switch`, `tooltip`, `select`, `tabs`, `textarea`, `checkbox`, `radio-group`, `separator`, `spinner`, and `searchbox` imports in V6 point to `components/clauseiq-v6/orbit-ui/*`.
...
- V6 compatibility adapters remain in `components/clauseiq-v6/orbit-ui/*` where replacing every call site with direct `@orbit` props would be higher risk than the visual migration itself.
