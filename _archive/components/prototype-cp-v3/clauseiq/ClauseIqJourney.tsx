...
import { Button as OrbitButton, Card, FA, FaIcon, LinkText, ToggleCard } from "@orbit-cp";
...
import {
  ...
  SelectTrigger,
  SelectValue,
} from "@/components/prototype-cp-v3/clauseiq/orbit-ui/select";
import { StateCard, type CardState } from "@/components/prototype-cp-v3/clauseiq/StateCard";
import {
...
} from "@/components/prototype-cp-v3/clauseiq/ClauseIqWorkflow";
import {
  AnalysisCard,
  ResultsContent,
  SupplierOutputsPanel,
} from "@/components/prototype-cp-v3/clauseiq/supplier-results";
import type {
  ResultsLayout,
  SupplierOutputSelection,
} from "@/components/prototype-cp-v3/clauseiq/supplier-results/types";
  ...
  return <div className={`relative ${mappingOpen ? "z-50" : "z-0"}`} aria-label="Supplier">
    ...
    {mappingOpen && <div id="detected-supplier-picker" className="absolute right-0 top-full z-40 max-h-[calc(100vh-6rem)] w-[min(360px,calc(100vw-2rem))] overflow-y-auto rounded-orbit-lg border border-orbit-border bg-orbit-card p-orbit-base shadow-orbit-md" style={{ marginTop: "calc(-1 * var(--orbit-space-xs))" }} role="dialog" aria-label={manualAddOpen ? "Add Supplier Manually" : "Change Mapped Supplier"} aria-modal="false">
      {manualAddOpen ? <>
        ...
        <Input value={supplierSearch} onChange={(event) => setSupplierSearch(event.target.value)} placeholder="Search or type new supplier name" className="prototype-cp-v3-supplier-picker-input mt-orbit-s h-9 w-full" aria-label="Search or type new supplier name" autoFocus />
...
export function PrototypeControl({ workflow }: { workflow: ClauseIqWorkflow }) {
  ...
  return <div className="relative" aria-label="Prototype tools">
    {open && <div className="absolute bottom-9 left-0 w-full rounded-orbit-md border border-[var(--orbit-color-sidenav-divider)] bg-[var(--orbit-color-sidenav-active-bg)] p-orbit-s text-[var(--orbit-color-sidenav-text)] shadow-lg"><label className="block text-orbit-xs v6-orbit-weight-medium text-orbit-inverse" htmlFor="supplier-detection-sim">Supplier detection preview</label><Select value={workflow.detectionOutcome} onValueChange={(value) => workflow.actions.setDetectionOutcome(value as typeof workflow.detectionOutcome)}><SelectTrigger id="supplier-detection-sim" className="mt-orbit-xs h-8 w-full bg-orbit-card text-orbit-xs clauseiq-v6-select-left prototype-cp-v3-simulation-select" aria-label="Supplier detection simulation"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="known">A · detected & known</SelectItem><SelectItem value="wrong-match">A′ · wrong match</SelectItem><SelectItem value="new">B · detected, not in list</SelectItem><SelectItem value="missing">C · not detected</SelectItem></SelectContent></Select><Button type="button" variant="outline" className="mt-orbit-s h-8 w-full border-[var(--orbit-color-sidenav-divider)] bg-transparent text-orbit-inverse hover:bg-[var(--orbit-color-sidenav-hover-bg)]" onClick={openInspector}>Inspect layout</Button></div>}
    <Button type="button" variant="outline" className="prototype-cp-v3-design-control-trigger h-7 w-full justify-center gap-orbit-xs border-[var(--orbit-color-sidenav-divider)] px-orbit-base text-orbit-xs text-orbit-inverse" onClick={() => setOpen((current) => !current)} aria-expanded={open}>Prototype tools</Button>
  </div>;
}
