...
import ClauseIQV7 from "./ClauseIQV7";
import { CIQ_DEFAULT_PLAYBOOK } from "@/lib/clauseiq-v7-data";
...
import { mockInitiative } from "@/data/mock-clauseiq-v7";
...
describe("ClauseIQ V7 flow", () => {
  ...
  it("renders the output-panel route with the first-run output by default", () => {
    renderClauseIQ("/clauseiq-v7/output-panel", { forceResults: true, resultsLayout: "output-panel" });
  ...
  it("shows Met and Not Met metadata once previous analysis exists", () => {
    renderClauseIQ("/clauseiq-v7/output-panel?resultScenario=history", {
      forceResults: true,
  ...
  it("opens the negotiated re-analysis outcome route for a later supplier output", () => {
    const { container } = render(
      <MemoryRouter initialEntries={["/clauseiq-v7/output-panel?resultScenario=history"]}>
        <TooltipProvider>
          <Routes>
            <Route
              path="/clauseiq-v7/output-panel"
              element={<ClauseIQV7 forceResults resultsLayout="output-panel" />}
            />
            <Route path="/initiatives-v7" element={<LocationEcho />} />
          </Routes>
        </TooltipProvider>
      </MemoryRouter>,
    );
    ...
    expect(route).toContain("/initiatives-v7?");
  ...
  it("opens the dashboard from the completed output-panel analysis card", () => {
    render(
      <MemoryRouter initialEntries={["/clauseiq-v7/output-panel"]}>
        <TooltipProvider>
          <Routes>
            <Route
              path="/clauseiq-v7/output-panel"
  ...
  it("starts a fresh workflow from the next-action analyse-another-initiative action", () => {
    renderClauseIQ("/clauseiq-v7?view=results");
  ...
  it("lets rerun analyses skip optional benchmark fields before upload", () => {
    renderClauseIQ("/clauseiq-v7/output-panel?rerun=upload", {
      forceResults: true,
      resultsLayout: "output-panel",
    });
  ...
  it("keeps previous output and the New Analysis divider after rerun completion", async () => {
    vi.useFakeTimers();
    const { container } = renderClauseIQ("/clauseiq-v7/output-panel", {
      forceResults: true,
      resultsLayout: "output-panel",
    });
  ...
  it("renders direct results routes with the default playbook selected", () => {
    const { container } = renderClauseIQ("/clauseiq-v7?view=results");
