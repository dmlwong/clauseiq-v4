...
import {
  ...
  BookOpen, Scale, ClipboardList, BadgeCheck, ChevronUp, ChevronDown, Upload,
} from "lucide-react";
import { LinkText, Text } from "@orbit";
import { Button } from "@/components/clauseiq-v5/orbit-ui/button";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/clauseiq-v5/orbit-ui/tabs";
import { showV5OrbitToast as toast } from "@/components/clauseiq-v5/V5OrbitToast";
import { V5ResponsiveTestingShell } from "@/components/clauseiq-v5/V5ResponsiveTestingShell";
...
type ResponsiveResultsTab = "result" | "actions";
...
interface ClauseIQResponsiveTestingProps {
  forceResults?: boolean;
  resultsLayout?: ResultsLayout;
}

export default function ClauseIQResponsiveTesting({ forceResults = false, resultsLayout = "accordion" }: ClauseIQResponsiveTestingProps) {
  ...
  const [responsiveResultsTab, setResponsiveResultsTab] = useState<ResponsiveResultsTab>("result");
  const [supplierOutputNavOpen, setSupplierOutputNavOpen] = useState(false);
  ...
  useEffect(() => {
    if (!forceResults && searchParams.get("view") === "results" && !rerunUploadFromRoute) {
      navigate("/clauseiq-responsive-testing/output-panel", { replace: true });
      return;
    }
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      setStep("results");
      if (!forceResults) {
        navigate("/clauseiq-responsive-testing/output-panel");
      }
    }, PROCESSING_MS);
  ...
  const handleStartAnotherInitiative = () => {
    ...
    setStep("welcome");
    navigate("/clauseiq-responsive-testing");
  };
  ...
  const showRunAgainUpload = () => {
    ...
    if (!forceResults) {
      ...
      navigate(`/clauseiq-responsive-testing/output-panel?${nextParams.toString()}`);
    }
  ...
  return (
    <V5ResponsiveTestingShell
      title="Responsive Testing"
      subtitle="ClauseIQ v5 layout sandbox"
      mainClassName="clauseiq-v5-canvas-grid"
      subheader={
        outputPanelResultsVisible ? (
          <ResponsiveSupplierOutputNav
            open={supplierOutputNavOpen}
            onOpenChange={setSupplierOutputNavOpen}
      ...
      subheaderClassName="clauseiq-responsive-supplier-output-subheader"
      headerRight={
        resultsVisible ? (
      ...
      rightPanel={
        <div className="clauseiq-responsive-supplier-output-sidebar h-full p-orbit-base">
          <SupplierOutputsPanel
            initiative={supplierOutputInitiative}
            outputState={supplierOutputPanelState}
            onRunAgain={showRunAgainUpload}
    ...
    >
      <div
        className={cn(
          ...
          "pb-orbit-xxl",
          "max-w-[640px]",
          outputPanelResultsVisible && "clauseiq-responsive-output-panel-flow",
        )}
      >
            {/* Welcome */}
            <StateCard
              state={welcomeState}
              className={outputPanelResultsVisible ? "clauseiq-responsive-context-card" : undefined}
            >
            ...
            {resultsVisible && (
              <>
                {outputPanelResultsVisible && (
                  <ResponsiveResultsHub
                    activeTab={responsiveResultsTab}
                    onTabChange={setResponsiveResultsTab}
                    initiative={resultsInitiative}
                    selectedAnalysisParameters={selectedAnalysisParameters}
                    completedRerunAnalysis={completedRerunAnalysis}
                    completedRerunSupplier={rerunSupplier}
                    completedRerunAnalysisParameters={completedRerunAnalysisParameters}
                    rerunUploadVisible={rerunUploadVisible}
                    rerunSelectedParameter={rerunSelectedParameter}
                    rerunProcessing={rerunProcessing}
                    selectedParameter={selectedParameter}
                    file={file}
                    completedMilestoneIds={completedMilestoneIds}
                    initiativeCompleted={initiativeCompleted}
                    showPostAnalysisActions={showPostAnalysisActions}
                    onRunAgain={showRunAgainUpload}
                    onViewResult={handleViewResult}
                ...
                <div ref={resultRef} className="clauseiq-responsive-desktop-results-section space-y-orbit-base">
                ...
                <div className="clauseiq-responsive-end-spacer h-[304px]" aria-hidden="true" />
              </div>
              </>
            )}
...
function ResponsiveSupplierOutputNav({
  open,
  onOpenChange,
  initiative,
  outputState,
  ...
  const counts = getSupplierOutputCounts(initiative);
  const description =
    outputState === "processing"
      ? "Analysis in progress"
      : counts.outputCount === 0
        ? "No supplier outputs yet"
        : `${counts.supplierCount} ${counts.supplierCount === 1 ? "supplier" : "suppliers"} · ${counts.outputCount} ${counts.outputCount === 1 ? "output" : "outputs"}`;

  return (
    <nav className="clauseiq-responsive-supplier-output-nav" aria-label="Supplier output navigation">
      <button
        type="button"
        className="clauseiq-responsive-supplier-output-toggle"
        aria-expanded={open}
        aria-controls="clauseiq-responsive-supplier-output-panel"
        onClick={() => onOpenChange(!open)}
      >
        <span className="min-w-0">
          <span className="block truncate text-xs text-muted-foreground">Supplier Outputs</span>
          <span className="block truncate text-sm v5-orbit-weight-medium text-foreground">{description}</span>
        </span>
        <ChevronDown className={cn("h-4 w-4 shrink-0 transition-transform", open && "rotate-180")} aria-hidden="true" />
      </button>

      {open && (
        <div id="clauseiq-responsive-supplier-output-panel" className="clauseiq-responsive-supplier-output-panel">
          <SupplierOutputsPanel
            initiative={initiative}
...
function ResponsiveResultsHub({
  ...
  activeTab: ResponsiveResultsTab;
  onTabChange: (tab: ResponsiveResultsTab) => void;
  initiative: Initiative;
  selectedAnalysisParameters: AnalysisParameterItem[];
  completedRerunAnalysis: ClauseAnalysis | null;
  completedRerunSupplier?: Initiative["suppliers"][number];
  completedRerunAnalysisParameters: AnalysisParameterItem[];
  rerunUploadVisible: boolean;
  rerunSelectedParameter: AnalysisParameterSelection | null;
  rerunProcessing: boolean;
  ...
  const latestRow =
    completedRerunAnalysis && completedRerunSupplier
      ? { supplier: completedRerunSupplier, analysis: completedRerunAnalysis }
      : getLatestAnalysisRow(initiative);
  const latestAnalysisParameters =
    completedRerunAnalysis && completedRerunSupplier
      ? completedRerunAnalysisParameters
      : selectedAnalysisParameters;

  const handleTabChange = (value: string) => {
    if (value === "result" || value === "actions") {
      onTabChange(value);
    }
  };

  const showUploadCard = rerunUploadVisible && hasCompleteAnalysisParameters(rerunSelectedParameter);

  return (
    <section className="clauseiq-responsive-results-hub" aria-label="Responsive results hub">
      {latestRow ? (
        <StateCard state="default" className="clauseiq-responsive-results-summary">
          <div className="space-y-orbit-base">
            <div className="flex items-start justify-between gap-orbit-base">
              <div className="min-w-0">
                <p className="text-xs v5-orbit-weight-medium uppercase tracking-[0.08em] text-muted-foreground">
                  Latest Output
                </p>
                <h2 className="v5-orbit-heading-5 mt-orbit-xxs truncate">
                  {latestRow.supplier.name}
                </h2>
              </div>
              <span className="shrink-0 rounded-md border border-border bg-white px-orbit-s py-orbit-xxs text-xs v5-orbit-weight-medium text-primary">
                Completed
              </span>
            </div>

            <div className="rounded-md border border-border bg-muted/30 p-orbit-base">
              <p className="truncate text-sm v5-orbit-weight-medium text-foreground">
                {latestRow.analysis.fileName}
              </p>
              <p className="mt-orbit-xxs text-xs text-muted-foreground">
                {latestRow.analysis.clausesReviewed} clauses reviewed · {formatResponsiveHubDate(latestRow.analysis.analysedAt)}
              </p>
            </div>

            <DeviationPills deviations={latestRow.analysis.deviations} compact />
          </div>
        </StateCard>
      ) : (
        <StateCard state="default" className="clauseiq-responsive-results-summary">
          <div className="space-y-orbit-s">
            <h2 className="v5-orbit-heading-5">No analysis output yet</h2>
            <p className="text-sm text-muted-foreground">
              Completed analysis results will appear here.
            </p>
          </div>
        </StateCard>
      )}

      <Tabs value={activeTab} onValueChange={handleTabChange} className="clauseiq-responsive-results-tabs-shell">
        <div className="clauseiq-responsive-results-tabs">
          <TabsList className="clauseiq-responsive-results-tablist">
            <TabsTrigger value="result">Result</TabsTrigger>
            <TabsTrigger value="actions">Actions</TabsTrigger>
          </TabsList>
        </div>

        <TabsContent value="result" className="clauseiq-responsive-results-panel space-y-orbit-base">
          {latestRow ? (
            <AnalysisCard
              analysis={latestRow.analysis}
              ...
              isLatestOutput
              highlighted
              analysisParameters={latestAnalysisParameters}
            />
          ) : (
            <StateCard state="default">
              <p className="text-sm text-muted-foreground">No current result to show.</p>
            </StateCard>
          )}
        </TabsContent>

        <TabsContent value="actions" className="clauseiq-responsive-results-panel space-y-orbit-base">
          {!rerunUploadVisible && !rerunProcessing && (
            <StateCard state="default">
              <div className="flex items-start justify-between gap-orbit-base">
                <div className="min-w-0">
                  <h2 className="v5-orbit-heading-5">Run Another Analysis</h2>
                  <p className="mt-orbit-xs text-sm text-muted-foreground">
                    Compare a new contract output against the same initiative.
                  </p>
                </div>
          ...
          {rerunProcessing && (
            <StateCard state="active">
              ...
              <PlaybookDisclaimer variant="inline" parameter={rerunSelectedParameter ?? selectedParameter} />
              <p className="text-xs text-muted-foreground mt-orbit-s">
                The existing analysis history remains available in Supplier Outputs while this runs.
              </p>
            </StateCard>
          )}
      ...
      <div className="clauseiq-responsive-bottom-action-bar" aria-label="Primary result actions">
        <div className="clauseiq-responsive-bottom-action-inner">
...
function getLatestAnalysisRow(initiative: Initiative): {
  supplier: Initiative["suppliers"][number];
  analysis: ClauseAnalysis;
} | null {
  return initiative.suppliers
    .flatMap((supplier) => supplier.analyses.map((analysis) => ({ supplier, analysis })))
    .sort((a, b) => Date.parse(a.analysis.analysedAt) - Date.parse(b.analysis.analysedAt))
    .at(-1) ?? null;
}

function getSupplierOutputCounts(initiative: Initiative) {
  const suppliersWithOutputs = initiative.suppliers.filter((supplier) => supplier.analyses.length > 0);
  const outputCount = suppliersWithOutputs.reduce((total, supplier) => total + supplier.analyses.length, 0);

  return {
    supplierCount: suppliersWithOutputs.length,
    outputCount,
  };
}

function formatResponsiveHubDate(value: string) {
  ...
  return (
    <StateCard state="default">
      <div className="space-y-orbit-base">
        ...
        <section className="clauseiq-responsive-milestone-card rounded-lg border border-border bg-card p-orbit-base" aria-labelledby="v5-update-milestone-title">
          <div className="flex items-start justify-between gap-orbit-base">
          ...
          <div className="clauseiq-responsive-milestone-table mt-orbit-m rounded-lg border border-border">
                ...
                {NEXT_ACTION_MILESTONES.map((milestone) => {
                  ...
                  return (
                    <tr key={milestone.id} className="border-b border-border last:border-b-0">
                      <td data-label="Milestone" className="px-orbit-base py-orbit-base text-base text-foreground">{milestone.label}</td>
                      <td data-label="Due Date" className="px-orbit-base py-orbit-base text-base text-foreground">{milestone.dueDate}</td>
                      <td data-label="Status" className={cn("px-orbit-base py-orbit-base text-base", completed ? "v5-orbit-weight-medium text-success" : "text-foreground")}>
                        {completed ? "Completed" : "Pending"}
                      </td>
                      <td data-label="Action" className="px-orbit-base py-orbit-s">
                        {completed ? (
