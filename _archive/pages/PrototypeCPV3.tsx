...
import { PrototypeCPV3Shell } from "@/components/prototype-cp-v3/PrototypeCPV3Shell";
import { InitiativeModal } from "@/components/prototype-cp-v3/clauseiq/InitiativeModal";
import {
  CIQ_INITIATIVES,
  type CiqInitiative,
} from "@/lib/clauseiq-v6-data";
import { cn } from "@/lib/utils";
import { SupplierOutputsPanel } from "@/components/prototype-cp-v3/clauseiq/supplier-results";
import type { ResultsLayout, SupplierOutputSelection } from "@/components/prototype-cp-v3/clauseiq/supplier-results/types";
import {
  ClauseIqJourneyContent,
  PrototypeControl,
  type ClauseIqJourneyRefs,
} from "@/components/prototype-cp-v3/clauseiq/ClauseIqJourney";
import {
...
import { getSupplierJourney, saveSupplierJourney } from "@/components/prototype-cp-v3/clauseiq/supplierJourneyStore";
import {
  getSupplierOutputComparisonContext,
  mockInitiative,
} from "@/data/mock-clauseiq-v6";

interface PrototypeCPV3Props {
  forceResults?: boolean;
  resultsLayout?: ResultsLayout;
}

export default function PrototypeCPV3(props: PrototypeCPV3Props) {
  ...
  return <PrototypeCPV3Content key={scenarioKey} {...props} />;
}

function PrototypeCPV3Content({ forceResults = false, resultsLayout = "output-panel" }: PrototypeCPV3Props) {
  ...
  const clauseIqResultsReturnRoute = `/prototype-cp-v3${normalizedOutputPanelSearch ? `?${normalizedOutputPanelSearch}` : ""}`;
  ...
  const handleRunAgain = useCallback(() => {
    if (!forceResults) {
      ...
      navigate(`/prototype-cp-v3?${nextParams.toString()}`);
    }
  ...
  const handleViewResult = (selection?: SupplierOutputSelection) => {
    ...
    if (!context) {
      ...
      navigate(`/prototype-cp-v3/results?${fallbackParams.toString()}`);
      return;
    }
    ...
    navigate(`/prototype-cp-v3/results?${params.toString()}`);
  };
