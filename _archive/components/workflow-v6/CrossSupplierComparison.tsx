...
export function CrossSupplierComparison({ initiativeId, onBack, onOpenSupplier, onOpenContract }: Props) {
  ...
  const supplierColumns = [
    ...
    },
    {
      id: "supplier",
      header: "Supplier",
      render: (supplier: ReturnType<typeof deriveSupplierStats>) => <span className="v6-orbit-weight-medium">{supplier.name}</span>,
    },
    {
      id: "status",
      header: "Status",
    ...
    <div className="min-h-screen p-orbit-l">
      <div className="max-w-6xl mx-auto space-y-orbit-m">
        ...
        <header className="space-y-orbit-xs">
          <h1 className="v6-orbit-heading-2">Compare suppliers — {initiative.name}</h1>
        ...
        {compared.length >= 2 && (
          <div className="bg-card border border-primary/30 rounded-xl overflow-hidden">
            <div className="px-orbit-m py-orbit-base border-b border-border">
              <h2 className="v6-orbit-heading-strong">Side-by-side comparison ({compared.length})</h2>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm border-separate border-spacing-0 min-w-[640px]">
                <thead className="bg-muted/30">
                  <tr>
                    <th className="v6-orbit-heading-strong sticky left-0 z-10 min-w-[180px] border-b border-border bg-muted/30 px-orbit-base py-orbit-s text-left text-muted-foreground">Metric</th>
                    {compared.map((c) => (
                      <th key={c.id} className="v6-orbit-heading-strong min-w-[180px] border-b border-border px-orbit-base py-orbit-s text-left">{c.name}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
...
function Row({ label, children }: { label: string; children: React.ReactNode }) {
  ...
  return (
    <tr>
      <td className="px-orbit-base py-orbit-s text-xs v6-orbit-weight-semibold text-muted-foreground uppercase border-t border-border sticky left-0 bg-card z-10">
        {label}
      </td>
      {items.map((child, i) => (
