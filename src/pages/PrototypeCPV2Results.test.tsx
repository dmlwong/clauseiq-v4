...
describe("Prototype CP v2 result dashboard", () => {
  ...
  it("uses CP-specific storage for apply and undo recommendations", async () => {
    ...
    expect(localStorage.getItem("prototype-cp-v2-clause-decisions")).toContain("request-update");
  ...
  it("keeps the CP review and generate flow independent from v5", () => {
    ...
    expect(localStorage.getItem("prototype-cp-v2-generated-csv:sup-1:ct-1:first-analysis-demo:v1")).toBeTruthy();
    expect(localStorage.getItem("prototype-cp-generated-csv:sup-1:ct-1:first-analysis-demo:v1")).toBeNull();
    expect(localStorage.getItem("ciq-v5-generated-csv:sup-1:ct-1:first-analysis-demo:v1")).toBeNull();
  });

  it("switches to the equivalent Prototype CP result route", () => {
    renderCpResults();

    choosePrototype("Prototype CP");

    expect(screen.getByTestId("location")).toHaveTextContent("/prototype-cp/results");
    expect(screen.getByTestId("location")).toHaveTextContent("source=prototype-cp");
  });
